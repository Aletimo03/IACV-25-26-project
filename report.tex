...
%  Geometry, IPPE-square, and conditioning experiments
...
\title{Synthetic IPPE-Square Pose Estimation \\[0.3em]
       \large Geometry, Two-Pose Ambiguity, and Conditioning}
...
\begin{enumerate}[label=\textbf{Step \arabic*.}, leftmargin=*]
  ...
  \item Compare the custom primary candidate to OpenCV's
        \verb|SOLVEPNP_IPPE_SQUARE| in the noiseless case.
\end{enumerate}

The remaining experiments sweep the front-facing camera viewpoint and study
the reprojection Jacobian under Gaussian pixel noise.
...
% ============================================================
\section{IPPE-Square Pose Recovery and Validation}
% ============================================================
...
\end{equation}
The custom solver estimates $\mathbf{H}$ by DLT, evaluates its quotient-rule
Jacobian at the square centre, and applies the IPPE construction to obtain two
rotation candidates. Translation is then estimated linearly for each fixed
rotation. The two candidates are kept unpolished and ranked by pixel RMSE:
an unrestricted nonlinear refinement may converge both starts to the same
pose and incorrectly hide the planar ambiguity.

The custom implementation is compared in the noiseless synthetic scene with
OpenCV's \verb|solvePnPGeneric(..., SOLVEPNP_IPPE_SQUARE)|. Both use the same
canonical centred-square point order. OpenCV is a reference only; the custom
solver itself has no OpenCV dependency.

% ============================================================
\section{Jacobian and Uncertainty Analysis}
% ============================================================

Let $\mathbf{r}$ stack the eight scalar pixel residuals of the four corners.
For a local pose increment ordered as
\begin{equation}
...
For the viewpoint sweep, the camera centre moves on a fixed-radius,
front-facing arc from $0^\circ$ (fronto-parallel) to $75^\circ$, while always
looking at the marker centre. Both IPPE candidates and their reprojection
errors are recorded at each viewpoint and plotted against the viewing angle.
