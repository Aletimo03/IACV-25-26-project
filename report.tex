...
\end{equation}
Stacking all four corners gives a $12\times3$ system, solved in the
least-squares sense. Only two of the three rows per corner are independent, but
keeping all of them is numerically harmless and avoids forming a normal-equation
inverse by hand.
...
measure. The price is a slight difference from OpenCV on the mirror candidate,
quantified in Section~\ref{sec:step7}.
...
\midrule
1&custom&$6.4\cdot10^{-14}$&$0$&$2.8\cdot10^{-13}$\\
\rowcolor{lightbg} 2&custom&$3.33836$&$34.521$&$3.074$\\
...
Two observations are worth making. First, the two solvers agree on the primary
pose to $10^{-13}$ mm, which validates the custom implementation. Second, the
...
shows this is not a coincidence. The small residual gap between the two
solvers on the mirror candidate ($3.33836$ against $3.33825$ px) comes from
the internal refinement OpenCV applies and we deliberately do not.
...
\resultfigure{viewpoint_sweep.png}
