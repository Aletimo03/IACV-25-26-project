...
Let $\theta$ be the viewing angle, the angle between the marker normal and the
line joining the marker centre to the camera centre, so that $\theta = 0$ is a
fronto-parallel view. Measured on a $0.5$~m arc with a $0.1$~m square, the
following holds to machine precision:
...
\begin{equation}
  ...
  \label{eq:geometry}
\end{equation}
The interpretation is clean. The matrix $\Rtil$ has singular values $1$ and
$\cos\theta$: a plane seen obliquely is compressed by a factor $\cos\theta$
along one direction and left unchanged along the perpendicular one, which is
exactly foreshortening. And $\|\bvec\|$, the quantity the image derivative
cannot see, equals $\sin\theta$: it measures how far the plane is tilted out of
the image plane.
